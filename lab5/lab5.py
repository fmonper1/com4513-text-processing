...
test_sentence = ["The mathematician ran".split() ,
                 "The mathematician ran to the store".split(),
                 "The physicist ran to the store".split(),
                 "The philosopher thought about it".split(),
                 "The mathematician solved the open problem".split()]

trigrams = list()
for sent in test_sentence:
    sent.insert(0, "START")
    sent.insert(len(sent), "STOP")
...
flat_list = [item for sublist in test_sentence for item in sublist]
vocab = set(flat_list)
...
for epoch in range(epochs):
    ...
    for context, target in trigrams:
        ...
        if context == ['START', 'The'] or epoch == epochs-1:
            _, predict_label = torch.max(log_probs, 1)
            predict_word = ix_to_word[predict_label.item()]
            print('context {}, real word is {}, predict word is {}'.format(context, target, predict_word))
...
words, label = trigrams[0]
word = torch.LongTensor([word_to_ix[i] for i in words])
out = model(word)
_, predict_label = torch.max(out, 1)
predict_word = ix_to_word[predict_label.item()]
print('context is {}, real word is {}, predict word is {}'.format(words, label, predict_word))
